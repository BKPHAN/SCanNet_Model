...
class Data_test(data.Dataset):
    ...
    #     data_list_A = os.listdir(imgA_dir)
    #     data_list_B = os.listdir(imgB_dir)
    #     for it in data_list:
    #         if (it[-4:] == '.png'):
    ...
    def __init__(self, test_dir):
        ...
        for it_A, it_B in zip(data_list_A, data_list_B):
            if it_A[-4:] == '.png' and it_B[-4:] == '.png':
                img_A_path = os.path.join(imgA_dir, it_A)
                img_B_path = os.path.join(imgB_dir, it_B)
                self.imgs_A.append(io.imread(img_A_path))
                self.imgs_B.append(io.imread(img_B_path))
                self.mask_name_list.append(it_A)  # Hoặc it_B tùy thuộc bạn muốn lưu tên nào
        self.len = len(self.imgs_A)
